...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [users]: any = await pool.query(
    "SELECT id, name, image, username, bio FROM users WHERE username = ?",
    [username]
  )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <div className="flex items-start gap-6 mb-10">
          {user.image ? (
            <img src={user.image} className="w-20 h-20 rounded-full flex-shrink-0" alt={user.name}/>
          ) : (
            <div className="w-20 h-20 rounded-full bg-orange-100 flex items-center justify-center text-2xl font-medium text-orange-600 flex-shrink-0">
              {user.name?.charAt(0)}
            </div>
          )}
          <div>
