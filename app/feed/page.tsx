          ...
          <Link href={`/u/${post.author_username}`} className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                <span className="text-xs text-gray-400">@{post.author_username} · {timeAgo(post.created_at)}</span>
                {post.visibility === "followers" && (
                  <span className="text-xs bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full ml-1">👥 Followers</span>
                )}
              </div>
            </div>
          </Link>
