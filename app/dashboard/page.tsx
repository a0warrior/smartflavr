  ...
  return (
    <div
      ...
      <button
        ...
        className="text-gray-300 hover:text-red-400 text-xs transition flex-shrink-0">
        ✕
      </button>
    </div>
  )
...
export default function Dashboard() {
  ...
  async function toggleGroceryItem(itemId: number, checked: boolean) {
    ...
    setGroceryLists(prev => prev.map((list: any) =>
      list.id === activeGroceryList?.id
        ? { ...list, items: list.items.map((item: any) => item.id === itemId ? { ...item, checked: checked ? 1 : 0 } : item) }
        : list
    ))
  }
  ...
  async function deleteGroceryItem(itemId: number) {
    await fetch("/api/grocery-lists", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: activeGroceryList.id, deleteItemIds: [itemId] }),
    })
    const updated = {
      ...activeGroceryList,
      items: activeGroceryList.items.filter((i: any) => i.id !== itemId)
    }
  ...
  async function addItemToList() {
    ...
    await fetch("/api/grocery-lists", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: activeGroceryList.id, addItems: [editItemInput.trim()] }),
    })
    setEditItemInput("")
  ...
  async function handleGroceryDragEnd(event: any) {
    ...
    await fetch("/api/grocery-lists", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        id: activeGroceryList.id,
        reorderItems: newOrder.map((item: any, index: number) => ({ id: item.id, sort_order: index }))
      }),
    })
  }

  async function deleteGroceryList(id: number) {
    setGroceryListToDelete(id)
    setShowDeleteGroceryModal(true)
  }
  ...
  async function confirmDeleteGroceryList() {
    ...
    await fetch("/api/grocery-lists", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: groceryListToDelete }),
    })
    setGroceryLists(prev => prev.filter((l: any) => l.id !== groceryListToDelete))
    if (activeGroceryList?.id === groceryListToDelete) {
  ...
  async function createNewGroceryList() {
    ...
    await fetch("/api/grocery-lists", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: newListName.trim(), items: newListItems }),
    })
    await fetchGroceryLists()
    setShowNewGroceryModal(false)
    setNewListName("")
    setNewListItems([])
    setNewListItem("")
  ...
  async function createCookbook() {
    ...
    await fetch("/api/cookbooks", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, cover_emoji: emoji, cover_color: color, cover_image: coverImage, is_public: isPublic }),
    })
    setTitle("")
    setEmoji("📖")
    setColor("#F97316")
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {cookbooks.map((book: any) => (
            <div key={book.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden cursor-pointer hover:shadow-sm transition group relative">
              <div onClick={() => router.push(`/cookbook/${book.id}`)}
              ...
              <div className="p-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={e => { e.stopPropagation(); setEditingCookbook({ ...book }); setShowEditModal(true) }}
                    className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-gray-600 text-xs px-2 transition">✏️</button>
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {collaboratedCookbooks.map((book: any) => (
                <div key={book.id} onClick={() => router.push(`/cookbook/${book.id}`)}
        ...
        <div className="mt-10">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => { setShowNewGroceryModal(true); setNewListName(""); setNewListItems([]); setNewListItem("") }}
              className="bg-orange-500 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-orange-600 transition">
              + New List
            </button>
          </div>
              ...
              {groceryLists.map((list: any) => {
                ...
                return (
                  <div key={list.id} className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center justify-between gap-4 hover:shadow-sm transition">
                    ...
                    <button onClick={() => deleteGroceryList(list.id)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-1">
              {editingListName ? (
                <input
                  ...
                  onBlur={async () => {
                    if (listNameInput.trim() && listNameInput !== activeGroceryList.name) {
                      await fetch("/api/grocery-lists", {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ id: activeGroceryList.id, name: listNameInput.trim() }),
                      })
                  ...
                  autoFocus
                  className="text-lg font-medium border-b border-orange-300 outline-none flex-1 mr-2"
                />
              ) : (
                <h2
                  onClick={() => { setListNameInput(activeGroceryList.name); setEditingListName(true) }}
                  className="text-lg font-medium cursor-pointer hover:text-orange-500 transition flex-1 mr-2"
                  title="Click to rename">
                  {activeGroceryList.name} ✏️
                </h2>
              )}
              <button onClick={() => deleteGroceryList(activeGroceryList.id)}
            ...
            <p className="text-xs text-gray-400 mb-3">
              {activeGroceryList.items?.filter((i: any) => i.checked).length} of {activeGroceryList.items?.length} items checked
            </p>
            <div className="flex gap-2 mb-4">
              <a href="https://www.hy-vee.com/aisles-online/" target="_blank" rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 py-2 bg-red-50 hover:bg-red-100 border border-red-100 rounded-xl transition">
                <span className="text-base">🛒</span>
                <span className="text-xs font-medium text-red-700">Hy-Vee</span>
              </a>
              <a href="https://www.walmart.com" target="_blank" rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-100 rounded-xl transition">
                <span className="text-base">🛍️</span>
                <span className="text-xs font-medium text-blue-700">Walmart</span>
              </a>
              <a href="https://shop.fareway.com" target="_blank" rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 py-2 bg-green-50 hover:bg-green-100 border border-green-100 rounded-xl transition">
                <span className="text-base">🥩</span>
                <span className="text-xs font-medium text-green-700">Fareway</span>
              </a>
            </div>
            <div className="flex gap-2 mb-4">
              <input
            ...
            <button onClick={() => { setShowGroceryListModal(false); setActiveGroceryList(null); setEditItemInput("") }}
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <input value={newListItem} onChange={e => setNewListItem(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && addItemToNewList()}
                  placeholder="e.g. 2 cups chicken broth"
                  className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none"/>
                <button onClick={addItemToNewList}
                  className="px-4 py-2 bg-orange-500 text-white rounded-xl text-sm font-medium hover:bg-orange-600">
                  Add
                </button>
              </div>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 py-2 px-3 bg-gray-50 rounded-xl group">
                      <span className="text-sm flex-1 text-gray-700">
                        {measurement ? <><span className="font-semibold">{measurement}</span>{rest ? ` ${rest}` : ""}</> : item}
                      </span>
                      <button onClick={() => setNewListItems(prev => prev.filter((_, idx) => idx !== i))}
                        className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-400 text-xs transition">✕</button>
                    </div>
                  )
                })}
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="mb-4">
              ...
              <div onClick={() => document.getElementById("cover-upload-new")?.click()}
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl text-sm border transition ${isPublic === 0 ? "bg-gray-900 text-white border-gray-900" : "border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
                  🔒 Private
                </button>
                <button
                  onClick={() => setIsPublic(1)}
                  className={`flex-1 py-2 rounded-xl text-sm border transition ${isPublic === 1 ? "bg-green-500 text-white border-green-500" : "border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
                  🌍 Public
                </button>
              </div>
            </div>
      ...
      {showEditModal && editingCookbook && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4">
              <label className="text-sm text-gray-500 mb-1 block">Cover image (optional)</label>
              <div onClick={() => document.getElementById("cover-upload-edit")?.click()}
                className="border-2 border-dashed border-gray-100 rounded-xl h-24 flex items-center justify-center cursor-pointer hover:bg-gray-50 overflow-hidden">
            ...
            <div className="mb-6">
              <label className="text-sm text-gray-500 mb-2 block">Visibility</label>
              <div className="flex gap-3">
                <button onClick={() => setEditingCookbook({ ...editingCookbook, is_public: 0 })}
                ...
                <button onClick={() => setEditingCookbook({ ...editingCookbook, is_public: 1 })}
      ...
      {showDeleteGroceryModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setShowDeleteGroceryModal(false); setGroceryListToDelete(null) }}
                className="flex-1 border border-gray-200 rounded-xl py-2 text-sm text-gray-500 hover:bg-gray-50">
                Cancel
              </button>
              <button
                onClick={confirmDeleteGroceryList}
      ...
      {showDeleteCookbookModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setShowDeleteCookbookModal(false); setCookbookToDelete(null) }}
                className="flex-1 border border-gray-200 rounded-xl py-2 text-sm text-gray-500 hover:bg-gray-50">
                Cancel
              </button>
              <button
                onClick={confirmDeleteCookbook}
                className="flex-1 bg-red-500 text-white rounded-xl py-2 text-sm font-medium hover:bg-red-600">
                Delete
              </button>
            </div>
      ...
      {cropImage && (
        <ImageCropper image={cropImage} aspect={16 / 9} onCrop={handleCropDone} onCancel={() => setCropImage("")}/>
      )}
    </div>
  )
}