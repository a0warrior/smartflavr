...
export async function GET(req: Request) {
  ...
  GROUP BY posts.id ORDER BY posts.created_at DESC LIMIT 50`

  const followingQuery = base + `
  AND (posts.user_id = ? OR posts.user_id IN (SELECT following_id FROM follows WHERE follower_id = ?))
  GROUP BY posts.id ORDER BY posts.created_at DESC LIMIT 50`
...
export async function POST(req: Request) {
  ...
  const [currentUser] = await pool.query(
    "SELECT id, post_timeout_until FROM users WHERE email = ?",
    [session.user.email]
  ) as any[]
